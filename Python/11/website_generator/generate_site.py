...
FEATURE_PATTERNS = {
    ...
    "range-based for loop": [
        r"\bfor\s*\(\s*(?:const\s+)?(?:auto|[A-Za-z_]\w*(?:::\w+)*)\s*[&*]?\s+\w+\s*:",
    ],
    "move semantics": [
        r"\bstd::move\s*\(",
        r"\bstd::forward\s*[<(]",
    ...
    "std::chrono": [r"\bstd::chrono::"],
    "std::regex": [
        r"\bstd::regex\b",
        r"\bstd::smatch\b",
        r"\bstd::regex_search\b",
        r"\bstd::regex_match\b",
    ],
    "auto type inference": [r"\bauto\s+\w+", r"\bconst\s+auto\b"],
    "constexpr": [r"\bconstexpr\s+\w+"],
    "scoped enum (enum class)": [r"\benum\s+class\s+\w+"],
    "variadic templates": [r"template\s*<[^>]*\.\.\."],
    "std::initializer_list": [r"\bstd::initializer_list\b"],
    "nullptr": [r"\bnullptr\b"],
    "static_assert": [r"\bstatic_assert\s*\("],
    "override / final": [r"\boverride\b", r"\bfinal\b"],
    "decltype": [r"\bdecltype\s*\("],
    "using type alias": [r"\busing\s+\w+\s*="],
    "std::array": [r"\bstd::array\b"],
    "std::tuple": [r"\bstd::tuple\b", r"\bstd::make_tuple\b", r"\bstd::tie\b"],
    "unordered containers": [r"\bstd::unordered_map\b", r"\bstd::unordered_set\b"],
    "noexcept": [r"\bnoexcept\b"],
    "defaulted / deleted functions": [r"=\s*default\s*;", r"=\s*delete\s*;"],
    "raw string literals": [r'\bR"\w*\('],
    "std::function": [r"\bstd::function\b"],
    "std::bind": [r"\bstd::bind\s*\("],
    "std::to_string": [r"\bstd::to_string\s*\("],
}
...
<head>
  ...
  <title>C++11 Examples Documentation</title>
...
<body>
  <header class=\"topbar\">
    <div>
      <h1>C++11 Examples Documentation</h1>
      <p id=\"siteMeta\"></p>
    </div>
  ...
  <main class=\"layout\">
    ...
    <section class=\"content\" id=\"content\">
      <div class=\"welcome\">
        <h2>Project Index</h2>
        <p>This site documents Jared Bruni's C++11 examples repository and provides generated walkthroughs for each source file.</p>
        <p>
          GitHub: <a class=\"ext-link\" href=\"https://github.com/lostjared/cplusplus11.Examples/\" target=\"_blank\" rel=\"noopener noreferrer\">https://github.com/lostjared/cplusplus11.Examples/</a>
        </p>
        <h3>Clone the Repository</h3>
        <pre><code>git clone https://github.com/lostjared/cplusplus11.Examples.git
cd cplusplus11.Examples</code></pre>
        <h3>Build Notes</h3>
        <pre><code># macOS
make

# Linux
make -f Makefile.linux</code></pre>
...
def derive_topic(rel_path: str) -> str:
    ...
    if len(parts) <= 1:
        return "general C++11 language features"
    folder = parts[0]
...
def summarize_file(rel_path: str, header_comment: str, features: List[str], functions: List[str], classes: List[str]) -> str:
    ...
    if not elements:
        return f"This file appears to be a support artifact for {topic}, included in the C++11 examples set."
