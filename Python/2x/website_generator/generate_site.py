...
FEATURE_PATTERNS = {
    ...
    "ranges": [r"\bstd::ranges::", r"\bstd::views::", r"#include\s*<ranges>"],
    "coroutines": [r"\bco_await\b", r"\bco_return\b", r"\bco_yield\b"],
    "consteval": [r"\bconsteval\b"],
    "constinit": [r"\bconstinit\b"],
    "three-way comparison (<=>)": [r"<=>"],
    "modules": [
        r"^\s*(?:export\s+)?module\s+\w+",
        r"^\s*export\s+import\b",
        r"^\s*import\s+<\w+>",
    ],
    ...
    "designated initializers": [r"\{\s*\.\w+\s*="],
    "using enum": [r"\busing\s+enum\b"],
    "std::source_location": [r"\bstd::source_location\b"],
    "std::print / println": [r"\bstd::print\b", r"\bstd::println\b"],
    "contains()": [r"\.contains\s*\("],
    "starts_with / ends_with": [r"\.starts_with\s*\(", r"\.ends_with\s*\("],
    "std::bit_cast": [r"\bstd::bit_cast\b"],
    "std::to_array": [r"\bstd::to_array\b"],
    "char8_t": [r"\bchar8_t\b"],
    "std::erase / erase_if": [r"\bstd::erase\b", r"\bstd::erase_if\b"],
    "template lambdas": [r"(?<!\w)\[[&=\w\s,*]*\]\s*<[^>]+>\s*\("],
    "std::midpoint / lerp": [r"\bstd::midpoint\b", r"\bstd::lerp\b"],
}
...
<head>
  ...
  <title>C++20/C++23 Examples Documentation</title>
...
<body>
  <header class=\"topbar\">
    <div>
      <h1>C++20/C++23 Examples Documentation</h1>
      <p id=\"siteMeta\"></p>
    </div>
  ...
  <main class=\"layout\">
    ...
    <section class=\"content\" id=\"content\">
      <div class=\"welcome\">
        <h2>Project Index</h2>
        <p>This site documents Jared Bruni's C++20/C++23 examples repository and provides generated walkthroughs for each source file.</p>
        <p>
          GitHub: <a class=\"ext-link\" href=\"https://github.com/lostjared/cplusplus2x.Examples/\" target=\"_blank\" rel=\"noopener noreferrer\">https://github.com/lostjared/cplusplus2x.Examples/</a>
        </p>
        <h3>Clone the Repository</h3>
        <pre><code>git clone https://github.com/lostjared/cplusplus2x.Examples.git
      cd cplusplus2x.Examples</code></pre>
        <h3>Build Notes</h3>
        <pre><code># Build inside an example directory
      make

      # Standards covered in this repo
      C++20 and C++23</code></pre>
...
def derive_topic(rel_path: str) -> str:
    ...
    if len(parts) <= 1:
        return "general C++20/C++23 language features"
    folder = parts[0]
...
def summarize_file(rel_path: str, header_comment: str, features: List[str], functions: List[str], classes: List[str]) -> str:
    ...
    if not elements:
        return f"This file appears to be a support artifact for {topic}, included in the C++20/C++23 examples set."
